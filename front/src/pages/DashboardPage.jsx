...
import { FiUsers, FiClock, FiAlertCircle, FiActivity, FiPieChart, FiBarChart2, FiDownload } from 'react-icons/fi';
...
const TicketStatusCard = ({ title, count, icon: Icon, color, link }) => (
  <div className="bg-white rounded-lg shadow p-5 flex">
    <div className={`rounded-full ${color} p-3 mr-4 flex items-center justify-center`}>
    ...
    <div>
      ...
      <div className="flex items-baseline">
        ...
        <Link to={link} className="ml-3 text-sm text-primary-600 hover:text-primary-800">
          View all
        </Link>
      </div>
    </div>
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (cachedData && cacheExpiry && new Date().getTime() < parseInt(cacheExpiry)) {
        const { tickets: cachedTickets, departments: cachedDepts, users: cachedUsers } = JSON.parse(cachedData);
      ...
      sessionStorage.setItem(`${cacheKey}-expiry`, (new Date().getTime() + 5 * 60 * 1000).toString());
    } catch (err) {
...
const resolutionTimeData = (() => {
  ...
  console.log("DEBUG: Sample closed ticket:", closedTickets.length > 0 ? {
  ...
  closedTickets.forEach(ticket => {
    ...
    if (priority.includes('low') || priority === '1' || priority === 'p1') {
      mappedPriority = 'low';
    } else if (priority.includes('med') || priority === '2' || priority === 'p2') {
      mappedPriority = 'medium';
    } else if (priority.includes('high') || priority === '3' || priority === 'p3') {
      mappedPriority = 'high';
    } else if (priority.includes('urg') || priority === '4' || priority === 'p4') {
      mappedPriority = 'urgent';
    } else {
      mappedPriority = 'medium'; // Default
    }
    
    console.log(`DEBUG: Ticket #${ticket.id} resolution time: ${hours} hours, priority: ${mappedPriority}`);
  ...
  const result = Object.keys(priorityTimes)
    ...
    .map(priority => ({
      ...
      hours: Math.round(priorityTimes[priority] / priorityCounts[priority]) || 0,
...
const userPerformanceData = (() => {
  ...
  const exportChartData = (chartName, data) => {
    ...
    link.setAttribute("download", `${chartName}-${new Date().toISOString().slice(0,10)}.csv`);
    ...
    <div>
      ...
      <div className="mb-4 flex flex-wrap gap-2 items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <label htmlFor="timeRange" className="text-sm text-gray-600">Time period:</label>
          <select 
            id="timeRange"
            ...
            <div className="absolute right-0 mt-64 z-10 bg-white shadow-lg rounded-md overflow-hidden">
              <DateRangePicker
                ...
                onChange={item => {
                  ...
                  const diffTime = Math.abs(item.selection.endDate - item.selection.startDate);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              <button 
                onClick={() => exportChartData('ticket-volume', volumeTrendData)}
          ...
          <ResponsiveContainer width="100%" height={300}>
            <AreaChart data={volumeTrendData} margin={{ top: 10, right: 10, left: 10, bottom: 10 }}>
              <CartesianGrid strokeDasharray="3 3" vertical={false} />
              <XAxis dataKey="date" />
              <YAxis allowDecimals={false} />
              <Tooltip />
              <Legend />
              <Area type="monotone" dataKey="created" name="New Tickets" stroke="#3B82F6" fill="#93C5FD" />
              <Area type="monotone" dataKey="closed" name="Resolved Tickets" stroke="#22C55E" fill="#86EFAC" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
              ...
              >
                ...
                <Bar dataKey="value" fill="#9333EA">
                  {backlogAgeData.map((entry, index) => {
                    ...
                    const colors = ['#22C55E', '#93C5FD', '#FCD34D', '#FB923C', '#EF4444'];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-bold">Average Resolution Time by Priority</h3>
          ...
          <ResponsiveContainer width="100%" height={300}>
            {resolutionTimeData[0]?.name === 'No Data' ? (
              <div className="h-full w-full flex flex-col items-center justify-center text-gray-400">
                ...
                <p className="text-sm">Close some tickets to see average resolution times</p>
              </div>
            ) : (
              <BarChart
              ...
              >
                ...
                <Tooltip 
                  formatter={(value, name, props) => [`${value} hours (${props.payload.count} tickets)`, 'Resolution Time']}
                />
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <ResponsiveContainer width="100%" height={300}>
            ...
            >
              ...
              <Bar dataKey="escalated" name="Escalated" fill="#EF4444" stackId="a" />
      ...
      <div className="bg-white rounded-lg shadow">
        ...
        <div className="p-4">
          <RecentTickets 
            tickets={tickets
              .sort((a, b) => new Date(b.updated_at || b.creation_date) - new Date(a.updated_at || a.creation_date))
              .slice(0, 5)
            } 
          />
        </div>
        <div className="p-4 border-t text-center">
          <Link to="/tickets" className="text-primary-600 hover:text-primary-800 text-sm font-medium">
            View all tickets
          </Link>
        </div>
      </div>
